...
class WorldometersSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for country in countries:
            ...
            # absolute_url = response.urljoin(link)  # concatenating links with urljoin  ################## response.urljoin(link)
            ...
            yield response.follow(url=link, callback=self.parse_country, meta={'country':country_name}) #####relative link(not absolute)

    # Getting data inside the "link" website
    def parse_country(self, response):
        # Getting country names and each row element inside the population table
        country = response.request.meta['country']          ##################### meta
